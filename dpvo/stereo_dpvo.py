...
class DPVO:
    def __init__(self, cfg, network, ht=480, wd=640, viz=False):
        ...
        self.fmap1_rigt_ = torch.zeros(1, self.mem, 128, ht // 1, wd // 1, **kwargs)
        self.fmap2_rigt_ = torch.zeros(1, self.mem, 128, ht // 4, wd // 4, **kwargs)

        # feature pyramid
        self.pyramid = (self.fmap1_, self.fmap2_)

        self.pyramid_rigt = (self.fmap1_rigt_, self.fmap2_rigt_)
    ...
    def corr(self, coords, indicies=None):
        ...
        jj1 = jj % (self.mem)
        stereo_mask = (ii // self.M == jj)
        corr1 = altcorr.corr(self.gmap, self.pyramid[0], coords / 1, ii1, jj1, 3)
        corr2 = altcorr.corr(self.gmap, self.pyramid[1], coords / 4, ii1, jj1, 3)

        if True in stereo_mask:
            #stereo process
            corr1[:, stereo_mask == True] = altcorr.corr(self.gmap, self.pyramid_rigt[0], coords / 1, ii1, jj1, 3)[:, stereo_mask == True] 
            corr2[:, stereo_mask == True] = altcorr.corr(self.gmap, self.pyramid_rigt[1], coords / 4, ii1, jj1, 3)[:, stereo_mask == True] 
    ...
    def __call__(self, tstamp, image_left, image_rigt, intrinsics, tstamp_log):
        ...
        if self.viewer is not None:
            self.viewer.update_image(image_left)

        image_left = 2 * (image_left[None,None] / 255.0) - 0.5

        image_rigt = 2 * (image_rigt[None,None] / 255.0) - 0.5
        
        #left image
        with autocast(enabled=self.cfg.MIXED_PRECISION):
            fmap, gmap, imap, patches, _, clr = \
                self.network.patchify(image_left,
        ...
        #right image
        with autocast(enabled=self.cfg.MIXED_PRECISION):
            fmap_rigt = self.network.patchify.fnet(image_rigt) / 4.0
        ...
        self.fmap1_rigt_[:, self.n % self.mem] = F.avg_pool2d(fmap_rigt[0], 1, 1)
        self.fmap2_rigt_[:, self.n % self.mem] = F.avg_pool2d(fmap_rigt[0], 4, 4)
