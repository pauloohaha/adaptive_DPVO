...
def difficulty_gt_eval(traj_est, tstamps, traj_ref):
    ...
    est_xyz           = result.trajectories['traj'].positions_xyz[-10:]
    ref_xyz           = result.trajectories['reference'].positions_xyz[-10:]
    ...
    return max(change_of_drift)*50 #scalling factor
...
def run(cfg, network, adaptive_network, imagedir, calib, stride=1, viz=False, traj_ref=None):
    ...
    while 1:
        ...
        if t > cfg.KEYFRAME_THRESH:
            difficulty_level = difficulty_gt_eval(traj_est, tstamps, traj_ref)
            print("difficulty level:" + str(difficulty_level))
            if difficulty_level > cfg.RESET_CONFIG_THRES:
                adaptive_num_patch = 96

            if difficulty_level < cfg.REDUCE_CONFIG_THRES and adaptive_num_patch > cfg.MIN_PATCHES:
                adaptive_num_patch = adaptive_num_patch - 16
